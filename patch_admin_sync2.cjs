...
if (!content.includes('syncAllToPocketBase')) {
  content = content.replace('const [isFormVisible, setIsFormVisible] = useState(true);', 'const [isFormVisible, setIsFormVisible] = useState(true);\n' + syncFunc);
}
...
`;
if (!content.includes('Server/PocketBase Synchronization')) {
