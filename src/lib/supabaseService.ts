...
export async function extractFirebaseCollections(onProgress?: (colName: string, count: number) => void) {
  ...
  const targets = [
    { key: 'users', fn: () => pocketbaseService.getUsers() },
    { key: 'complaints', fn: () => pocketbaseService.getComplaints() },
    { key: 'clients', fn: () => pocketbaseService.getClients() },
    { key: 'chatGroups', fn: () => pb.collection('chat_groups').getFullList() },
    { key: 'chatMessages', fn: () => pb.collection('chat_messages').getFullList() },
    { key: 'notifications', fn: () => pb.collection('notifications').getFullList() },
    { key: 'monitorTargets', fn: () => pb.collection('monitor_targets').getFullList() },
    { key: 'ledgerSheets', fn: () => pocketbaseService.getLedgerSheets() }
  ];

  for (const t of targets) {
    try {
      if (onProgress) onProgress(t.key, -1);
      const list = await t.fn();
      (result as any)[t.key] = list;
      if (onProgress) onProgress(t.key, list.length);
    } catch (e) {
      console.warn(`Extraction info: Unable to read PocketBase collection [${t.key}].`, e);
  ...
  try {
    ...
    const months = await pocketbaseService.getBillingMonths();
    if (months) {
      months.forEach(item => {
        const parsedRows = item.rows || [];
        const displayId = item.id;
        const dealerId = item.dealerId || 'main';
