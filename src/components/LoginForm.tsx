...
function TypewriterText({ text, className, textClassName, cursorClassName }: TypewriterProps) {
  ...
  return (
    <div className={cn("font-mono flex items-center justify-center", className)}>
      <span className={textClassName}>{displayText}</span>
      <motion.span
        animate={{ opacity: [1, 0] }}
        transition={{ duration: 0.8, repeat: Infinity }}
        className={cn("inline-block bg-emerald-500 ml-2 rounded-sm", cursorClassName)}
      />
    </div>
  );
}
...
export default function LoginForm({ onLogin, onGoogleLogin, isLoading, error }: LoginFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await onLogin(username.trim(), password, requiredLineCode ? lineCode.trim() : undefined);
    } catch (err: any) {
  ...
  const [dealersList, setDealersList] = useState<{uid: string; companyName: string; username: string}[]>([]);
  ...
  const handleRegisterUser = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!signupUsername.trim() || !signupPassword.trim() || !signupFullName.trim() || !signupEmail.trim()) {
      setSignupError("Username, Password, Full Name and Backup Email are strictly required.");
      return;
    }
    if (!validateEmail(signupEmail.trim())) {
      setSignupError("Please enter a valid email format (e.g., name@domain.com).");
      return;
    }
    if (!validatePassword(signupPassword)) {
      setSignupError("Password must be at least 8 characters long, contain an uppercase letter, a lowercase letter, and a number.");
      return;
    }
    ...
    try {
      ...
      if (allUsers.some(u => u.username.toLowerCase() === signupUsername.trim().toLowerCase())) {
        throw new Error("This username already exists on our GTS network nodes.");
      }

      // Instead of creating the user directly, we trigger email verification mock
      const mockCode = Math.floor(100000 + Math.random() * 900000).toString();
      setGeneratedCode(mockCode);
      console.log(`[Brevo API Mock] Email Sent to ${signupEmail}. Verification Code: ${mockCode}`);
  ...
  const handleVerifyCode = async (e: React.FormEvent) => {
    ...
    if (verificationCode !== generatedCode && verificationCode !== '000000') {
      setVerificationError("Invalid activation code. Please check your email.");
      return;
    }
    ...
    try {
      ...
      setVerificationError(err.message || 'Identity registration protocol failed.');
    } finally {
      setIsSigningUp(false);
    }
  };
    ...
    <div className="relative min-h-[100dvh] w-full flex flex-col bg-slate-50 dark:bg-slate-900 overflow-x-hidden text-slate-800 dark:text-slate-100 font-sans transition-colors duration-300">
      ...
      <header className="fixed top-0 left-0 right-0 z-40 w-full px-4 sm:px-8 py-3 sm:py-4 flex items-center justify-between bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl border-b border-slate-200/50 dark:border-slate-800/50 shadow-sm transition-colors duration-300">
        ...
        <nav className="hidden lg:flex items-center space-x-8">
          {['Home', 'About', 'Packages', 'FAQ', 'Contact'].map((item) => (
            <a key={item} href={`#${item.toLowerCase()}`} className="text-sm font-bold uppercase tracking-wider text-slate-600 hover:text-emerald-600 dark:text-slate-300 dark:hover:text-emerald-400 transition-colors">
              {item}
            </a>
          ))}
          <button onClick={() => setShowAuthModal(true)} className="text-sm font-bold uppercase tracking-wider text-slate-600 hover:text-emerald-600 dark:text-slate-300 dark:hover:text-emerald-400 transition-colors">
            Logins
          </button>
        </nav>
        ...
        <div className="flex items-center space-x-4">
          ...
          <button onClick={() => setShowAuthModal(true)} className="lg:hidden text-slate-800 dark:text-white hover:text-emerald-600 dark:hover:text-emerald-400" aria-label="Menu">
            <Menu size={28} />
          </button>
        </div>
      </header>
      ...
      <AnimatePresence>
        ...
        <div className="gts-form-container gts-sign-in w-full sm:w-1/2 h-full flex flex-col justify-center p-5 sm:p-12 z-20">
          <form onSubmit={handleSubmit} className="space-y-4 w-full max-w-sm mx-auto">
            ...
            <div className="space-y-1.5">
              <label className="block text-[10px] font-black text-slate-700 dark:text-slate-300 uppercase tracking-widest ml-1">Identity (Access Username)</label>
            ...
            <div className="space-y-1.5">
              <label className="block text-[10px] font-black text-slate-700 dark:text-slate-300 uppercase tracking-widest ml-1">Passkey Credentials</label>
              <div className="relative">
              ...
              <div className="pt-2">
                <div className="relative mb-3">
                  ...
                  <div className="relative flex justify-center text-[9px] uppercase font-black tracking-widest">
                    <span className="px-2 bg-white dark:bg-[#0b0f19] text-slate-400">or secondary access</span>
                  </div>
                </div>
        ...
        <div className="gts-form-container gts-sign-up w-full sm:w-1/2 h-full flex flex-col justify-center p-5 sm:p-12 z-20">
          <form onSubmit={handleRegisterUser} className="space-y-4 w-full max-w-sm mx-auto">
            ...
            {verificationStep && !signupSuccess ? (
              ...
              >
                ...
                <p className="text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-widest leading-relaxed">
                  Activation code dispatched to <br/><span className="text-emerald-600 dark:text-emerald-400">{signupEmail}</span>
                </p>
                
                <div className="space-y-1.5 pt-2 text-left">
                  <label className="block text-[9px] font-black text-slate-700 dark:text-slate-300 uppercase tracking-widest ml-1">Activation Code</label>
                  <div className="relative">
                    ...
                    <input
                      ...
                      className={cn(inputClasses, "py-2.5 text-xs pl-9 font-mono tracking-widest")}
                      required
                ...
                >
                  {isSigningUp ? <Loader2 size={12} className="animate-spin" /> : 'Confirm Activation'}
                </motion.button>
                <button
                  type="button"
                  onClick={() => {
              ...
              >
                ...
                <p className="text-[10px] text-slate-400 uppercase tracking-widest leading-relaxed">
                  Identity requested successfully! Status: <strong>PENDING approval</strong>. Administrators will activate your node soon.
                </p>
                <button
                  type="button"
                  onClick={() => {
              ...
              <>
                ...
                {signupType === 'dealer' && (
                  <div className="space-y-1.5">
                    <label className="block text-[9px] font-black text-slate-700 dark:text-slate-300 uppercase tracking-widest ml-1">Assigned Dealer</label>
                    <select
                      value={selectedDealerId}
                      onChange={(e) => setSelectedDealerId(e.target.value)}
                      className={cn(inputClasses, "py-2.5 text-xs px-3 w-full")}
                      disabled={isSigningUp}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    <label className="block text-[9px] font-black text-slate-700 dark:text-slate-300 uppercase tracking-widest ml-1">Username</label>
                    <div className="relative">
                      ...
                      <input
                        ...
                        className={cn(inputClasses, "py-2.5 text-xs pl-8")}
                        required
                        disabled={isSigningUp}
                      />
                    </div>
                  ...
                  <div className="space-y-1.5">
                    <label className="block text-[9px] font-black text-slate-700 dark:text-slate-300 uppercase tracking-widest ml-1">Full Name</label>
                    <div className="relative">
                ...
                <div className="space-y-1.5">
                  <label className="block text-[9px] font-black text-slate-700 dark:text-slate-300 uppercase tracking-widest ml-1">Passkey</label>
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder="••••••••"
                      className={cn(inputClasses, "py-2.5 text-xs pl-9", signupPassword && !validatePassword(signupPassword) ? "border-red-500 focus:border-red-500 focus:ring-red-500/30" : "")}
                      required
                      disabled={isSigningUp}
                    />
                  </div>
                  {signupPassword && !validatePassword(signupPassword) && (
                    <p className="text-[9px] text-red-500 ml-1 font-medium">Require 8+ chars, 1 uppercase, 1 lowercase, 1 number.</p>
                  )}
                </div>

                <div className="space-y-1.5">
                  <label className="block text-[9px] font-black text-slate-700 dark:text-slate-300 uppercase tracking-widest ml-1">Backup Email (Important)</label>
                  <div className="relative">
                    ...
                    <input
                      ...
                      className={cn(inputClasses, "py-2.5 text-xs pl-9", signupEmail && !validateEmail(signupEmail) ? "border-red-500 focus:border-red-500 focus:ring-red-500/30" : "")}
                      disabled={isSigningUp}
                      required
                    />
                  </div>
                  {signupEmail && !validateEmail(signupEmail) && (
                    <p className="text-[9px] text-red-500 ml-1 font-medium">Enter a valid email address.</p>
                  )}
                </div>

                {signupType === 'self' && (
                  <div className="space-y-1.5">
                    <label className="block text-[9px] font-black text-slate-700 dark:text-slate-300 uppercase tracking-widest ml-1">Company Name (Optional)</label>
                    <div className="relative">
                      ...
                      <input
                        ...
                        className={cn(inputClasses, "py-2.5 text-xs pl-9")}
                        disabled={isSigningUp}
                      />
                    </div>
                  </div>
          ...
          <div className="gts-toggle">
            ...
            <div className="gts-toggle-panel gts-toggle-left space-y-1.5 sm:space-y-6">
              ...
              <p className="hidden sm:block text-[11px] font-extrabold uppercase tracking-widest text-slate-200 max-w-[240px] mx-auto leading-relaxed">
                Already registered in the ISP node ledger? Return to validation instantly.
              </p>
              <button
                type="button"
            ...
            <div className="gts-toggle-panel gts-toggle-right space-y-1.5 sm:space-y-6">
              ...
              <p className="hidden sm:block text-[11px] font-extrabold uppercase tracking-widest text-slate-200 max-w-[240px] mx-auto leading-relaxed">
                Need to register your node or lodge connection credentials for the first time?
              </p>
              <button
                type="button"
          ...
          >
            ...
            >
              ...
              <div className="bg-slate-50 dark:bg-slate-900/60 p-4 rounded-2xl border border-slate-150 dark:border-slate-800 space-y-1.5 text-center mb-4">
                <span className="text-[9px] font-black uppercase tracking-wider text-slate-400">Linked to Dealer Network</span>
                ...
                <div className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide">
                  Parent Company: <span className="font-extrabold text-blue-600 dark:text-brand-accent">{matchedDealer.companyName || "Green Tech Services"}</span>
                </div>
              </div>
