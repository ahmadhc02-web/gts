async function wipe() {
  const loginRes = await fetch('http://167.233.41.7:8090/api/collections/_superusers/auth-with-password', {
    method: 'POST',
  ...
  while(true) {
    const res = await fetch('http://167.233.41.7:8090/api/collections/billing_rows/records?page=1&perPage=500', {
    ...
    const promises = data.items.map(r => 
      fetch(`http://167.233.41.7:8090/api/collections/billing_rows/records/${r.id}`, {
        method: 'DELETE',
        headers: { 'Authorization': token }
      })
    );
